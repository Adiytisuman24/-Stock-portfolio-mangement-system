...
const StockChart: React.FC<StockChartProps> = ({ data, symbol, loading }) => {
  ...
  const chartData = data
    ...
    .map((item, index) => ({
      ...item,
      displayTime: formatDistanceToNow(new Date(item.ts), { addSuffix: true }),
      fullDate: item.ts,
      index,
    }));
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg">
          <p className="font-semibold text-gray-900 mb-2">{data.displayTime}</p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span className="font-semibold text-green-600">
              ${Math.max(...data.map(d => d.high || 0)).toFixed(2)}
            </span>
          </div>
          <div>
            <span className="text-gray-600">Period Low: </span>
            <span className="font-semibold text-red-600">
              ${Math.min(...data.map(d => d.low || Infinity)).toFixed(2)}
            </span>
          </div>
        </div>
      )}
